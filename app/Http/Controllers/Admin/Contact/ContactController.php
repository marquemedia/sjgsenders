...
class ContactController extends Controller
{
    ...
    */ 
    ...
    public function search(Request $request) {
        ...
        $contacts           = Contact::whereNull("user_id");
        $groups             = Group::whereNull("user_id")->where("status", Group::ACTIVE)->pluck("name", "id")->toArray();
        $attributes         = json_decode($this->general->contact_attributes);
        ...
        $contactAttributes = $this->general->contact_attributes;
        $title             = 'Contact Search';
        return view('admin.contact.index', compact('title', 'contacts', 'data', 'groups', 'contactAttributes', 'filteredAttributes'));
    }
    ...
    */ 
    ...
    public function index($id = null): View {
        ...
        $contacts           = $id ? Contact::whereNull("user_id")->where("group_id",$id)->latest()->paginate(paginateNumber()) : Contact::whereNull("user_id")->latest()->paginate(paginateNumber());
        $contactAttributes  = $this->general->contact_attributes;
        $attributes         = json_decode($this->general->contact_attributes);
        $filteredAttributes = collect($attributes)->filter(function ($attribute) {
            return $attribute->status === true;
        ...
        $groups             = Group::whereNull("user_id")->where("status", Group::ACTIVE)->pluck("name", "id")->toArray();
        return view('admin.contact.index', compact('title', 'contacts', 'filteredAttributes', 'groups', 'contactAttributes'));
    }
    ...
    public function create():View {
        ...
        $groups             = Group::whereNull("user_id")->where("status", Group::ACTIVE)->pluck("name", "id")->toArray();
        return view('admin.contact.create', compact('title', 'filteredAttributes', 'groups', 'contactAttributes'));
    }
