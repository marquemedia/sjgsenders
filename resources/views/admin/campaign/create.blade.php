                            ...
                            <div>
                                <div class="mb-3">
                                    ...
                                    <div class="form-item">
                                        ...
                                        @elseif($channel == \App\Models\Campaign::SMS)
                                            <div class="speech-to-text" id="messageBox">
                                                <textarea class="form-control message" name="message" id="sms-message" placeholder="{{translate('Enter SMS Content')}}  @php echo "\nTo initiate text spinner type {Hello|Hi|Hola} to you, {Mr.|Mrs.|Ms.} {Lucia|Jimmy|Arnold}"@endphp" aria-describedby="text-to-speech-icon">{{old('message')}}</textarea>
                                                <span class="voice-icon" id="text-to-speech-icon">
                                                    <i class='fa fa-microphone text-to-speech-toggle'></i>
                                                </span>
                                            </div>
	...
	(function($){
        ...
        $(document).ready(function () {
            ...
            $('.mail_gateway_type').change(function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{route('admin.gateway.select2', 'email')}}",
                    data:{
                        'type' : selectedType,
                    },
                    dataType: 'json',
            ...
            $('.sms_gateway_type').change(function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{route('admin.gateway.select2', 'sms')}}",
                    data:{
                        'type' : selectedType,
                    },
                    dataType: 'json',
                ...
                success: function (response) {
                    ...
                    if (response.templates.length > 0) {
                        ...
                        $.each(response.templates, function (index, template) {
                            ...
                            $('#whatsapp_template_id').append('<option value="' + template.id + '">' + template.name + '('+ template.language_code+')'+ '</option>');
